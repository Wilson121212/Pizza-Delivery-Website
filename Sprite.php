...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['addSprite'])) {
        $quantity = isset($_POST['SpriteQty']) ? (int)$_POST['SpriteQty'] : 0;
        $itemID = 29;
        ...
        if ($quantity > 0) {
            ...
            if ($stmt->execute()) {
                $successMessage = "$quantity x Sprite has been successfully added.";
            } else {
...
<head>
    ...
    <title>Sprite - PengYu's Pizzas</title>
...
<body>
    ...
    <section class="menu">
        ...
        <div class="pizza-detail">
                ...
                <h2>Sprite</h2>
                <p>Zesty lemon-lime Sprite, perfectly carbonated and refreshing.</p>
                <p>4.7 ⭐ (280)</p>
                <p>$2.50</p>
            </div>

            <form method="post" id="customizeForm1" class="grey-box">
                <label for="SpriteQty">Quantity:</label>
                <input type="number" min="0" id="SpriteQty" name="SpriteQty" value="0">
                <button type="submit" class="add-to-cart-button" name="addSprite">Add to Cart</button>
            </form>
        </main>
    </section>
