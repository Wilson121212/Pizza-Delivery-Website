...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['addHimalayanFries'])) {
        $quantity = isset($_POST['HimalayanFriesQty']) ? (int)$_POST['HimalayanFriesQty'] : 0;
        $itemID = 26;
        ...
        if ($quantity > 0) {
            ...
            if ($stmt->execute()) {
                $successMessage = "$quantity x Himalayan Fries has been successfully added.";
            } else {
...
<head>
    ...
    <title>Himalayan Fries - PengYu's Pizzas</title>
...
<body>
    ...
    <section class="menu">
        ...
        <div class="pizza-detail">
                <img src="HimalayanFries.jpg" alt="Himalayan Fries">
                <h2>Himalayan Fries</h2>
                <p>Crispy French fries, perfectly salted with fresh himalayan salt and served hot.</p>
                <p>4.6 ⭐ (190)</p>
                <p>$3.90</p>
            </div>

            <form method="post" id="customizeForm1" class="grey-box">
                <label for="HimalayanFriesQty">Quantity:</label>
                <input type="number" min="0" id="HimalayanFriesQty" name="HimalayanFriesQty" value="0">
                <button type="submit" class="add-to-cart-button" name="addHimalayanFries">Add to Cart</button>
            </form>
        </main>
    </section>
