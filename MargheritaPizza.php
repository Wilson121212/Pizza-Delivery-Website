...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['addMargheritta'])) {
        // Determine itemID based on selected size
        $size = $_POST['Margheritasize'];
        $thickness = $_POST['Margheritathickness'];
        $sauce = $_POST['Margheritasauce'];
        $quantity = isset($_POST['MargheritaQty']) ? (int)$_POST['MargheritaQty'] : 0;
        ...
        if ($quantity > 0) {
            ...
            if ($stmt->execute()) {
                $successMessage = "$quantity x Margherita Pizza (Size: $size, Thickness: $thickness, Sauce: $sauce) has been successfully added.";
            } else {
...
<body>
    ...
    <main>
        <div class="pizza-detail">
            ...
            <p>Classic Margherita with fresh tomatoes, creamy mozzarella, and fragrant basil,
                 all on a crisp, golden crust. Simple, timeless, and full of flavor!</p>
            <p>Rating: 4.5 ⭐ (260)</p>
            <p>Price: $17.90</p>
        </div>        
    ...
    <form id="customizeForm" method="post">
        ...
        <select id="sauce" name="Margheritasauce">
        ...
        <select id="thickness" name="Margheritathickness">
        ...
        <select id="size" name="Margheritasize">
            <option value="small">Small - 10"</option>
            <option value="medium">Medium - 12"</option>
            <option value="large">Large - 14"</option>
        </select>
        
        <label for="MargheritaQty">Quantity:</label>
        <input type="number" min="0" id="MargheritaQty" 
        name="MargheritaQty" value="0" >
        
        <button type="submit" class="add-to-cart-button" name="addMargheritta">Add to Cart</button>
    </form>
    </main>
    </section>
